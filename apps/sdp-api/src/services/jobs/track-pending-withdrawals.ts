...
async function reconcileSubmitted(
  ...
  // group across ticks.
  const gatewayHandle = gatewayAuth.kind === "token" ? gatewayAuth.handle : undefined;
  const [status] = await withGatewayRpc(env, withdrawal.gateway_url, gatewayHandle, (gatewayRpc) =>
