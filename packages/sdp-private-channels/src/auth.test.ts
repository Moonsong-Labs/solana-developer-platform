...
describe("createAuthClient", () => {
  it("register posts credentials and returns the user", async () => {
    ...
    const user = await client.register({ username: "sdp_abc", password: "pw" });
  ...
  it("login returns the token", async () => {
    const { fetchImpl } = stubFetch(json({ token: "jwt-123" }));
    const client = createAuthClient(BASE, { fetchImpl });
  ...
  it("classifies a 409 as CONFLICT with the server's error message", async () => {
    const { fetchImpl } = stubFetch(json({ error: "username already taken" }, 409));
    const client = createAuthClient(BASE, { fetchImpl });

    await expect(client.register({ username: "u", password: "p" })).rejects.toMatchObject({
      code: "CONFLICT",
  ...
  it("health reports ok + status without throwing on non-2xx", async () => {
